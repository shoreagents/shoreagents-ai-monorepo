...
export default function TimeTracking() {
  ...
  // Check for late clock-in and show modal (ONLY ONCE per shift!)
  useEffect(() => {
    if (isClockedIn && activeEntry?.wasLate && activeEntry?.lateBy && activeEntry?.id) {
      // Check if we've already shown this modal for this time entry
      const shownKey = `late-modal-shown-${activeEntry.id}`
      const alreadyShown = localStorage.getItem(shownKey)
      
      if (!alreadyShown) {
      setLateMinutes(activeEntry.lateBy)
      setShowLateModal(true)
        // Mark as shown for this time entry
        localStorage.setItem(shownKey, 'true')
      }
    }
  }, [isClockedIn, activeEntry?.wasLate || false, activeEntry?.lateBy || 0, activeEntry?.id])

  // Check for early clock-in and show modal (ONLY ONCE per shift!)
  useEffect(() => {
    if (isClockedIn && activeEntry?.wasEarly && activeEntry?.earlyBy && activeEntry?.id) {
      // Check if we've already shown this modal for this time entry
      const shownKey = `early-modal-shown-${activeEntry.id}`
      const alreadyShown = localStorage.getItem(shownKey)
              ...
              <div className="text-center">
               ...
               <Button
                 ...
                 disabled={
                   isClockingIn || 
                   isClockingOut || 
                   (!isClockedIn && !canClockIn) // Only check canClockIn when trying to clock IN
                 }
                 size="lg"
