...
export default function TicketList({ tickets, onTicketClick, onStatusChange }: TicketListProps) {
  ...
  const getAssigneeName = (ticket: Ticket) => {
    if (ticket.staffUser) return ticket.staffUser.name
    if (ticket.managementUser) return ticket.managementUser.name
    if (ticket.clientUser) return ticket.clientUser.name
    return "Unassigned"
  }

  const getCreatorName = (ticket: Ticket) => {
    if (ticket.createdByType === "STAFF" && ticket.staffUser) return ticket.staffUser.name
    if (ticket.createdByType === "MANAGEMENT" && ticket.managementUser) return ticket.managementUser.name
    if (ticket.createdByType === "CLIENT" && ticket.clientUser) return ticket.clientUser.name
    return "Unknown"
  }
